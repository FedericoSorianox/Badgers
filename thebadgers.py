...
def generar_id_unico(base="elemento"):
    """Generar ID único para elementos de Streamlit"""
    timestamp = int(time.time() * 1000)
    random_part = uuid.uuid4().hex[:6]
    return f"{base}_{timestamp}_{random_part}"
def guardar_inventario_db(nombre, inventario_data):
    """Guardar producto de inventario en la base de datos"""
    ...
    try:
        cursor.execute('''
            ...
            (nombre, precio_unitario, valor_unitario, stock_semanal)
            VALUES (?, ?, ?, ?)
        ''', (
            nombre,
            inventario_data.get('precio_unitario'),
            inventario_data.get('valor_unitario'),
            inventario_data.get('stock_semanal')
        ))
        conn.commit()
        print(f"✅ Producto {nombre} guardado en BD")
        return True
...
def init_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS pagos (
            ...
            tipo_cuota TEXT,
            fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (ci) REFERENCES socios (ci)
        )
    ''')
...
def cargar_datos_desde_db():
    ...
    try:
        ...
        for row in socios_db:
            ci = row[0]
            st.session_state.socios[ci] = {
                'nombre': row[1],
                'ci': ci,
                'celular': row[2] or '',
                'contacto_emergencia': row[3] or '',
                'emergencia_movil': row[4] or '',
                'fecha_nacimiento': row[5] or '',
                'tipo_cuota': row[6] or 'Libre - $2000',
                'enfermedades': row[7] or '',
                'comentarios': row[8] or '',
                'foto': row[9]
            }
        ...
        for row in pagos_db:
            pago_id = row[0]
            st.session_state.pagos[pago_id] = {
                'ci': row[1],
                'mes': row[2],
                'año': row[3],
                'monto': row[4],
        ...
        for row in gastos_db:
            ...
            st.session_state.gastos[gasto_id] = {
                ...
                'es_recurrente': bool(row[6]),
                'frecuencia': row[7] or 'Única vez'
            }
    ...
    try:
        cursor.execute('''
            ...
            (ci, nombre, celular, contacto_emergencia, emergencia_movil, 
             fecha_nacimiento, tipo_cuota, enfermedades, comentarios, foto)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            ci,
            socio_data.get('nombre', ''),
            socio_data.get('celular', ''),
            socio_data.get('contacto_emergencia', ''),
            socio_data.get('emergencia_movil', ''),
            socio_data.get('fecha_nacimiento', ''),
            socio_data.get('tipo_cuota', 'Libre - $2000'),
            socio_data.get('enfermedades', ''),
            socio_data.get('comentarios', ''),
            socio_data.get('foto')
        ))
        conn.commit()
        return True
    except sqlite3.Error as e:
        st.error(f"Error guardando socio: {e}")
        return False
    finally:
        conn.close()

def guardar_pago_db(pago_id, pago_data):
    ...
    try:
        cursor.execute('''
            ...
            (id, ci, mes, año, monto, fecha_pago, tipo_cuota)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            pago_id,
            pago_data.get('ci'),
            ...
            pago_data.get('tipo_cuota')
        ))
        conn.commit()
        return True
    except sqlite3.Error as e:
        st.error(f"Error guardando pago: {e}")
        return False
    finally:
        conn.close()

def guardar_gasto_db(gasto_id, gasto_data):
    ...
    try:
        cursor.execute('''
            ...
            (id, concepto, monto, fecha, descripcion, categoria, es_recurrente, frecuencia)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            gasto_id,
            gasto_data.get('concepto'),
            gasto_data.get('monto'),
            gasto_data.get('fecha'),
            gasto_data.get('descripcion', ''),
            gasto_data.get('categoria', 'Otros'),
            gasto_data.get('es_recurrente', False),
            gasto_data.get('frecuencia', 'Única vez')
        ))
        conn.commit()
        return True
    except sqlite3.Error as e:
...
def eliminar_socio_db(ci):
    ...
    try:
        ...
        cursor.execute('DELETE FROM pagos WHERE ci = ?', (ci,))
        # Eliminar socio
...
def eliminar_gasto_db(gasto_id):
...
# Modificar la función inicializar_datos()
def inicializar_datos():
  #  """Inicializar datos desde base de datos o crear datos de ejemplo"""
    ...
    # Si no hay datos en la base de datos, crear datos de ejemplo
    if not st.session_state.socios:
        print("📋 No hay datos en BD, creando datos de ejemplo...")
        ...
        # Guardar socios en BD
        for ci, socio in st.session_state.socios.items():
            guardar_socio_db(ci, socio)
        ...
        # Guardar pagos en BD
        for pago_id, pago in st.session_state.pagos.items():
            guardar_pago_db(pago_id, pago)
        ...
        for nombre, producto in st.session_state.inventario.items():
            guardar_inventario_db(nombre, producto)
        ...
        # Guardar gastos en BD
        for gasto_id, gasto in st.session_state.gastos.items():
            guardar_gasto_db(gasto_id, gasto)
        
        print("✅ Datos de ejemplo creados y guardados en BD")
    else:
        print(f"✅ Datos cargados desde BD: {len(st.session_state.socios)} socios")
  #  """Inicializar datos desde base de datos o crear datos de ejemplo"""
...
def mostrar_agregar_socio():
    ...
    if resultado:
        ...
        try:
            ...
            if guardar_socio_db(ci, socio_data):
                st.success(f"✅ Socio {socio_data['nombre']} guardado correctamente")
                
                if socio_data.get('foto'):
                    st.success(f"📸 CON FOTO guardada")
                else:
                    st.info("📸 SIN FOTO")
                ...
                # Si falla la BD, remover de session_state
                del st.session_state.socios[ci]
        ...
        if st.session_state.inventario:
            ...
            with col2:
                ...
                if st.button("🔄 Actualizar Stock", key="actualizar_stock"):
                    # Actualizar el stock en session_state y base de datos
                    st.session_state.inventario[producto_actualizar]['stock_semanal'] = nuevo_stock
                    guardar_inventario_db(producto_actualizar, st.session_state.inventario[producto_actualizar])
                    st.success(f"Stock actualizado para {producto_actualizar}")
                    st.rerun()
   
        st.info("No hay productos en inventario")
    
    with tab2:
        """Solo la parte del tab2 corregida para inventario"""
    st.subheader("Agregar Producto")
    
    # Form key único
    form_key = generar_id_unico("form_producto")
    
    with st.form(form_key):
        nombre_key = generar_id_unico("nombre_producto")
        nombre_producto = st.text_input("Nombre del Producto*", 
                                      placeholder="Ej: Guantes de boxeo", key=nombre_key)
        
        col1, col2 = st.columns(2)
        with col1:
            precio_key = generar_id_unico("precio_unitario")
            precio_unitario = st.number_input("Precio Unitario (Venta)*", 
                                            min_value=0.0, value=0.0, step=0.01, key=precio_key)
            
            valor_key = generar_id_unico("valor_unitario")
            valor_unitario = st.number_input("Valor Unitario (Costo)*", 
                                           min_value=0.0, value=0.0, step=0.01, key=valor_key)
        with col2:
            stock_key = generar_id_unico("stock_semanal")
            stock_semanal = st.number_input("Stock Semanal*", 
                                          min_value=0, value=0, key=stock_key)
        ...
        submit_key = generar_id_unico("submit_producto")
        ...
        if submitted:
            if not nombre_producto.strip():
                ...
                st.session_state.inventario[nombre_producto] = producto_data
                
                if guardar_inventario_db(nombre_producto, producto_data):
                    ganancia = precio_unitario - valor_unitario
                    st.success(f"Producto {nombre_producto} agregado correctamente")
                    st.info(f"Ganancia unitaria: ${ganancia:,.2f}")
                    st.rerun()
                else:
                    del st.session_state.inventario[nombre_producto]
                    st.error("❌ Error guardando producto en base de datos")
def show_gastos():
...
def mostrar_lista_gastos():
    """Mostrar lista de gastos con keys únicos en filtros"""
    ...
    if st.session_state.gastos:
        ...
        with col1:
            ...
            categoria_key = generar_id_unico("categoria_filtro")
            categoria_filtro = st.selectbox("Filtrar por categoría:", categorias_unicas, key=categoria_key)
        
        with col2:
            tipos_gasto = ["Todos", "Recurrentes", "Únicos"]
            tipo_key = generar_id_unico("tipo_filtro")
            tipo_filtro = st.selectbox("Filtrar por tipo:", tipos_gasto, key=tipo_key)
        
        with col3:
            orden_key = generar_id_unico("orden_filtro")
            orden = st.selectbox("Ordenar por:", ["Fecha (más reciente)", "Fecha (más antigua)", "Monto (mayor)", "Monto (menor)"], key=orden_key)
...
def mostrar_agregar_gasto():
    ...
    # Form key único
    form_key = generar_id_unico("form_gasto")
    
    with st.form(form_key):
        col1, col2 = st.columns(2)
        
        with col1:
            concepto_key = generar_id_unico("concepto")
            concepto = st.text_input("Concepto del Gasto*", 
                                   placeholder="Ej: Alquiler local", key=concepto_key)
            
            monto_key = generar_id_unico("monto")
            monto = st.number_input("Monto*", min_value=0.0, value=0.0, 
                                  step=0.01, key=monto_key)
            
            fecha_key = generar_id_unico("fecha")
            fecha = st.date_input("Fecha del Gasto*", value=date.today(), key=fecha_key)
        ...
        with col2:
            ...
            categoria_key = generar_id_unico("categoria")
            categoria = st.selectbox("Categoría*", categorias, key=categoria_key)
            
            recurrente_key = generar_id_unico("recurrente")
            es_recurrente = st.checkbox("¿Es un gasto recurrente?", 
                                      value=False, key=recurrente_key)
            
            if es_recurrente:
                frecuencias = ["Mensual", "Trimestral", "Semestral", "Anual"]
                frecuencia_key = generar_id_unico("frecuencia")
                frecuencia = st.selectbox("Frecuencia*", frecuencias, key=frecuencia_key)
            else:
                frecuencia = "Única vez"
        
        descripcion_key = generar_id_unico("descripcion")
        ...
        submit_key = generar_id_unico("submit_gasto")
        ...
        if submitted:
            if not concepto.strip():
                ...
                st.session_state.gastos[gasto_id] = gasto_data
                
                try:
                    if guardar_gasto_db(gasto_id, gasto_data):
                        ...
                        st.error("❌ Error guardando gasto en base de datos")
                except Exception as e:
                    st.error(f"❌ Error: {str(e)}")
                    if gasto_id in st.session_state.gastos:
                        del st.session_state.gastos[gasto_id]
   # """Mostrar formulario para agregar gasto"""
   # st.subheader("➕ Agregar Nuevo Gasto")
    ...
    with st.form(f"form_editar_gasto_{gasto_id}"):
        ...
        if submitted:
            if not nuevo_concepto.strip():
                ...
                st.rerun()
    def actualizar_gasto_en_bd(gasto_id, nuevo_concepto, nuevo_monto, nueva_fecha, nueva_categoria, nueva_descripcion, nuevo_es_recurrente, nueva_frecuencia):
        """Actualizar gasto en base de datos"""
        gasto_data_actualizado = {
            'concepto': nuevo_concepto.strip(),
            'monto': nuevo_monto,
            'fecha': str(nueva_fecha),
            'categoria': nueva_categoria,
        ...
        st.session_state.gastos[gasto_id] = gasto_data_actualizado
        
        # Actualizar en base de datos
        if guardar_gasto_db(gasto_id, gasto_data_actualizado):
            return True
        else:
            st.error("❌ Error actualizando en base de datos")
            return False
...
def mostrar_confirmacion_eliminar_gasto():
    ...
    with col1:
        confirmar_key = generar_id_unico("confirmar_del_gasto")
        if st.button("✅ Sí, eliminar", type="primary", key=confirmar_key):
            if eliminar_gasto_db(gasto_id):
                del st.session_state.gastos[gasto_id]
                del st.session_state.confirmar_eliminar_gasto
                st.success(f"✅ Gasto eliminado correctamente")
                st.rerun()
            else:
                st.error("❌ Error eliminando de la base de datos")
    
    with col3:
        cancelar_key = generar_id_unico("cancelar_del_gasto")
    ...
    def confirmar_eliminar_gasto(gasto_id):
...
def actualizar_socio_con_ci(resultado, ci_original):
    ...
    if ci_cambio:
        # Eliminar socio anterior de BD
        eliminar_socio_db(ci_original)
        ...
        for pago_key, pago in st.session_state.pagos.items():
            if pago.get('ci') == ci_original:
                # Eliminar pago anterior de BD
                eliminar_pago_db(pago_key)
                
                # Crear nuevo pago con nueva CI
                nueva_clave = pago_key.replace(ci_original, ci_nueva)
                pago['ci'] = ci_nueva
                pagos_actualizados[nueva_clave] = pago
                
                # Guardar pago actualizado en BD
                guardar_pago_db(nueva_clave, pago)
            else:
    ...
    if not guardar_socio_db(ci_nueva, socio_data):
        st.error("❌ Error guardando cambios en base de datos")
    """Actualiza los datos del socio con manejo de cambio de CI"""
    socio_data = resultado['socio_data']
    ci_cambio = resultado.get('ci_cambio', False)
    ci_nueva = resultado.get('ci_nueva', ci_original)
...
def eliminar_socio_completo(ci_eliminar):
    ...
    # Contar pagos que se van a eliminar
    pagos_relacionados = sum(1 for pago in st.session_state.pagos.values() 
                           if pago.get('ci') == ci_eliminar)
    
    # Eliminar de la base de datos (incluye pagos relacionados)
    if eliminar_socio_db(ci_eliminar):
        # Eliminar de session_state
        ...
        if pagos_relacionados > 0:
            st.info(f"📋 También se eliminaron {pagos_relacionados} pagos relacionados")
    else:
        st.error("❌ Error eliminando de la base de datos")
...
def show_pagos():
    ...
    with tab2:
        ...
        # USAR form_key único para evitar duplicados
        form_key = generar_id_unico("form_pago")
        ...
        with st.form(form_key):
            ...
            with col1:
                ...
                socio_key = generar_id_unico("socio_select")
                socio_selected = st.selectbox("Socio*", list(socios_options.keys()), key=socio_key)
                
                meses = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio',
                        'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre']
                
                # Key único para mes
                mes_key = generar_id_unico("mes_select")
                mes = st.selectbox("Mes*", meses, index=datetime.now().month - 1, key=mes_key)
                
            with col2:
                # Key único para año
                año_key = generar_id_unico("año_input")
                año = st.number_input("Año*", min_value=2020, max_value=2030, 
                                    value=datetime.now().year, key=año_key)
                
                # Key único para fecha
                fecha_key = generar_id_unico("fecha_input")
                fecha_pago = st.date_input("Fecha de Pago", value=date.today(), key=fecha_key)
                
                # Obtener monto por defecto basado en el tipo de cuota
                ...
                # Key único para monto
                monto_key = generar_id_unico("monto_input")
                monto = st.number_input("Monto", value=monto_default, min_value=0, key=monto_key)
            
            # Key único para submit button
            submit_key = generar_id_unico("submit_pago")
            ...
            if submitted:
                ...
                if pago_key in st.session_state.pagos:
                    st.warning("⚠️ Ya existe un pago para este socio en este mes/año")
                else:
                    socio_data = st.session_state.socios.get(ci_selected, {})
                    ...
                    st.session_state.pagos[pago_key] = pago_data
                    
                    try:
                        if guardar_pago_db(pago_key, pago_data):
                            st.success(f"✅ Pago de ${monto:,.0f} registrado para {socio_selected}")
                            st.balloons()
                            st.rerun()
                        else:
                            del st.session_state.pagos[pago_key]
                            st.error("❌ Error guardando pago en base de datos")
                    except Exception as e:
                        st.error(f"❌ Error guardando pago: {str(e)}")
                        if pago_key in st.session_state.pagos:
                            del st.session_state.pagos[pago_key]
    ...
    with tab3:
        ...
        # Key único para selectbox de año
        año_verificar_key = generar_id_unico("año_verificar")
        año_verificar = st.selectbox("Año a verificar", [2024, 2025, 2026], 
                                   index=1, key=año_verificar_key)
        ...
        #        meses = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio',
         #               'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre']
          ...
          #          # Obtener datos del socio
           ...
           #         # Guardar en base de datos (si tienes las funciones de BD implementadas)
           #         try:
           #             if 'guardar_pago_db' in globals():
           #                 if guardar_pago_db(pago_key, pago_data):
           #                     st.success(f"✅ Pago de ${monto:,.0f} registrado para {socio_selected}")
           #                     st.balloons()
           #                     st.rerun()
           ...
           #                     st.error("❌ Error guardando pago en base de datos")
           #             else:
           #                 # Si no hay funciones de BD, solo mostrar éxito
           #                 st.success(f"✅ Pago de ${monto:,.0f} registrado para {socio_selected}")
           #                 st.balloons()
           #                 st.rerun()
           #         except Exception as e:
           #             st.error(f"❌ Error guardando pago: {str(e)}")
           #             # Remover de session_state si hay error
           #             if pago_key in st.session_state.pagos:
           #                 del st.session_state.pagos[pago_key]
...
def crear_backup():
    ...
    try:
        ...
        backup_filename = f"academia_backup_{fecha}.db"
        shutil.copy2("academia.db", backup_filename)
...
# Agregar en el sidebar del debug
#def mostrar_opciones_backup():
 #   """Mostrar opciones de backup en el sidebar"""
  #  st.sidebar.markdown("---")
   # st.sidebar.subheader("💾 Backup")
    #
    #if st.sidebar.button("📥 Crear Backup"):
     #   crear_backup()
    #
    #if st.sidebar.button("📊 Info BD"):
     #   mostrar_info_bd()
   ...
   # if os.path.exists('academia.db'):
    #    conn = sqlite3.connect('academia.db')
     ...
     #   
      ...
      #      count_socios = cursor.fetchone()[0]
      #      
...
def main():
 #   """Función principal"""
    inicializar_datos()  # Ahora carga desde BD
    ...
    # AGREGAR KEY ÚNICO AL RADIO
    selected_page = st.sidebar.radio(
        "Selecciona una sección:",
        menu_options,
        index=0,
        key="menu_principal_radio"  # ← AGREGAR ESTA LÍNEA
    )
  ...
  #      st.write(f"**Socios:** {len(st.session_state.socios)}")
   #     st.write(f"**Pagos:** {len(st.session_state.pagos)}")
    #    st.write(f"**Gastos:** {len(st.session_state.gastos)}")
     #   st.write(f"**Inventario:** {len(st.session_state.inventario)}")
    ...
    # AGREGAR: Opciones de base de datos
#    mostrar_opciones_backup()
