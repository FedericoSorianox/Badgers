...
def cargar_todos_los_datos():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if socios_rows:
                socios_cols = [desc[0] for desc in cursor.description]
                data['socios'] = {row[0]: dict(zip(socios_cols, row)) for row in socios_rows}
            else:
                data['socios'] = {}
            ...
            if pagos_rows:
                pagos_cols = [desc[0] for desc in cursor.description]
                data['pagos'] = [dict(zip(pagos_cols, row)) for row in pagos_rows]
            else:
                data['pagos'] = []
            ...
            if inventario_rows:
                inventario_cols = [desc[0] for desc in cursor.description]
                data['inventario'] = [dict(zip(inventario_cols, row)) for row in inventario_rows]
            else:
                data['inventario'] = []
            ...
            if gastos_rows:
                gastos_cols = [desc[0] for desc in cursor.description]
                data['gastos'] = [dict(zip(gastos_cols, row)) for row in gastos_rows]
            else:
                data['gastos'] = []
...
def guardar_imagen_base64(imagen_file):
    ...
    try:
        ...
        max_size = (400, 400); image.thumbnail(max_size, Image.Resampling.LANCZOS)
...
def formulario_socio(socio_data=None, es_edicion=False):
    ...
    with st.form(key=f"form_socio_{ci_original or 'nuevo'}", clear_on_submit=True):
        ...
        if submitted:
            ...
            socio_actualizado = {
                'nombre': nombre.strip(), 'ci': final_ci,
                'celular': (celular or '').strip(), 'contacto_emergencia': (contacto_emergencia or '').strip(),
                'emergencia_movil': (emergencia_movil or '').strip(), 'fecha_nacimiento': str(fecha_nacimiento) if fecha_nacimiento else '',
                'tipo_cuota': tipo_cuota, 'enfermedades': (enfermedades or '').strip(),
                'comentarios': (comentarios or '').strip(), 'foto': foto_base64
            }
            query = """INSERT INTO socios (ci, nombre, celular, contacto_emergencia, emergencia_movil, fecha_nacimiento, tipo_cuota, enfermedades, comentarios, foto)
                       VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                       ON CONFLICT (ci) DO UPDATE SET
                       nombre = EXCLUDED.nombre, celular = EXCLUDED.celular, contacto_emergencia = EXCLUDED.contacto_emergencia,
                       emergencia_movil = EXCLUDED.emergencia_movil, fecha_nacimiento = EXCLUDED.fecha_nacimiento, tipo_cuota = EXCLUDED.tipo_cuota,
                       enfermedades = EXCLUDED.enfermedades, comentarios = EXCLUDED.comentarios, foto = EXCLUDED.foto;"""
            params = (final_ci, socio_actualizado['nombre'], socio_actualizado['celular'], socio_actualizado['contacto_emergencia'],
                      socio_actualizado['emergencia_movil'], socio_actualizado['fecha_nacimiento'] or None, socio_actualizado['tipo_cuota'],
                      socio_actualizado['enfermedades'], socio_actualizado['comentarios'], socio_actualizado['foto'])
            
            if db_execute(query, params):
                st.success(f"✅ Socio '{nombre}' {'actualizado' if es_edicion else 'guardado'}.")
                st.balloons(); cargar_todos_los_datos.clear(); st.session_state.edit_mode = {}; st.rerun()
...
def pagina_socios(app_data):
    ...
    if accion == "Agregar Nuevo":
        ...
        if not app_data.get('socios'):
            st.warning("No hay socios registrados."); return
        ...
        if app_data.get('socios'):
            ...
            csv = convert_df_to_csv(df_socios_export)
            st.download_button(label="📥 Descargar lista de socios como CSV", data=csv, file_name=f"socios_the_badgers_{datetime.now().strftime('%Y%m%d')}.csv", mime='text/csv')
        ...
        if uploaded_file:
            try:
                df_import = pd.read_csv(uploaded_file, dtype={'ci': str, 'celular': str, 'emergencia_movil': str})
                st.write("Previsualización de datos a importar:")
                ...
                if st.button("Confirmar Importación", type="primary"):
                    ...
                    for i, row in df_import.iterrows():
                        socio_data = row.to_dict()
                        if not socio_data.get('ci') or not socio_data.get('nombre'):
                            st.warning(f"Omitiendo fila {i+1} por falta de CI o nombre."); continue
                        
                        params_tuple = tuple(socio_data.get(col) for col in all_db_columns)
                        ...
                        if db_execute(query, params_tuple):
                            success_count += 1
                        
                        progress_bar.progress((i + 1) / total_rows)

                    st.success(f"¡Importación completada! {success_count} de {total_rows} socios fueron guardados/actualizados.")
...
def pagina_pagos(app_data):
    ...
    data = []
    for ci, socio in socios.items():
        fila = {"CI": ci, "Socio": socio['nombre']}
        pagos_socio = [p for p in app_data['pagos'] if p['ci'] == ci and p['año'] == año_actual]
        for i, mes_nombre in enumerate(meses):
            pago_mes = next((p for p in pagos_socio if p['mes'] == i + 1), None)
            fila[mes_nombre] = "✅" if pago_mes else "❌"
        data.append(fila)
...
def main():
    ...
    if 'app_initialized' not in st.session_state:
        ...
        st.session_state.confirm_delete_ci = None
    ...
    st.sidebar.markdown("---"); st.sidebar.info("Aplicación de gestión para la academia.")
    ...
    page_map.get(seleccion, pagina_dashboard)(app_data) if seleccion != "Administración" else pagina_administracion()
